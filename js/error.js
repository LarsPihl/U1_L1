...
function areaCalculations() {
	...
	length = Number(input1Elem.value);//Inmatningar omvandlas till nummer och sparas i nya variabler.
	width = Number(input2Elem.value);

	area = length * width;//Area för en rektangel.
	resultElem.innerHTML = "<p>Rektangelns area: " + area + "</p>";
	
	area = length * width / 2;//Area för en triangel.
	resultElem.innerHTML += "<p>Triangelns area: " + area + "</p>";
	
	area = 3.14 * length * width / 4;//Area för en ellips.
